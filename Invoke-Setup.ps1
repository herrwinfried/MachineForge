...
try {
    ...
    $WindowTitleSession = Get-I18n 'WindowTitleSession'
    $WindowTitleAdmin = Get-I18n 'WindowTitleAdmin'
    Set-WindowTitle -Title $WindowTitleSession
    ...
    $MatchedProfile = $null
    while ($true) {
        Set-WindowTitle -Title (Get-I18n 'WindowTitleSession')
        Clear-Host
...
`$WindowTitleAdmin = '$($WindowTitleAdmin -replace "'", "''")'
Set-WindowTitle -Title `$WindowTitleAdmin
...
"@
        ...
        $AdminScript += "`n} finally {`n"
        $AdminScript += "    Set-WindowTitle -Title `$OriginalWindowTitle`n"
        $AdminScript += "}`n"
